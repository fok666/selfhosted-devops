...
variable "runner_count_per_instance" {
  description = <<-EOT
    Number of GitHub runners per VM/instance (0 = auto-detect based on vCPU count).
...
variable "nsg_outbound_internet_access" {
  description = <<-EOT
    ...
    - Connecting to GitHub (api.github.com, github.com)
    ...
    - May break GitHub Runner functionality if not properly configured
