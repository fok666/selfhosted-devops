...
# =============================================================================
# This file contains all network-related resources for GitHub runners.
...
data "azurerm_virtual_network" "existing" {
  ...
  resource_group_name = var.existing_vnet_resource_group_name != "" ? var.existing_vnet_resource_group_name : azurerm_resource_group.runner.name
}

data "azurerm_subnet" "existing" {
  count                = var.create_subnet ? 0 : 1
  name                 = var.existing_subnet_name
  virtual_network_name = var.create_vnet ? azurerm_virtual_network.runner[0].name : var.existing_vnet_name
  resource_group_name  = var.create_vnet ? azurerm_resource_group.runner.name : (var.existing_vnet_resource_group_name != "" ? var.existing_vnet_resource_group_name : azurerm_resource_group.runner.name)
}

data "azurerm_network_security_group" "existing" {
  count               = var.create_nsg ? 0 : 1
  name                = var.existing_nsg_name
  resource_group_name = var.existing_nsg_resource_group_name != "" ? var.existing_nsg_resource_group_name : azurerm_resource_group.runner.name
}
...
locals {
  # VNet ID
  vnet_id = var.create_vnet ? azurerm_virtual_network.runner[0].id : data.azurerm_virtual_network.existing[0].id

  # Subnet ID
  subnet_id = var.create_subnet ? azurerm_subnet.runner[0].id : data.azurerm_subnet.existing[0].id

  # NSG ID
  nsg_id = var.create_nsg ? azurerm_network_security_group.runner[0].id : data.azurerm_network_security_group.existing[0].id
}
